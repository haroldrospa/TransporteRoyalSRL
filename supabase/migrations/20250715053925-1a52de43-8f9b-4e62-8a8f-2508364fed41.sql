...
SELECT 
  r.id as relacion_id,
  DATE(c.fecha_entrega) as fecha_relacion,
  ...
  ARRAY_AGG(CASE WHEN c.estado = 'Entregado' THEN c.numero_conduce END) FILTER (WHERE c.estado = 'Entregado') as conduces_entregados_lista
FROM conduces c
JOIN relaciones_conduces r ON r.nombre = c.relacion
WHERE c.relacion IS NOT NULL
GROUP BY r.id, DATE(c.fecha_entrega)
