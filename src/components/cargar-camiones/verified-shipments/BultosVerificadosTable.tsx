...
interface BultosVerificadosTableProps {
...
const BultosVerificadosTable = ({ 
  shipments, 
  onDeleteShipment,
  onDeleteAllShipments,
  onExportComplete,
  isDeleting
}: BultosVerificadosTableProps) => {
  // Filtrar solo bultos (scan_type === 'bulto')
  const bultoShipments = shipments.filter(shipment => shipment.scan_type === 'bulto');
  ...
  const {
    ...
    filteredShipments,
    conducesToDisplay
  } = useShipmentTable(bultoShipments);
          ...
          <TableBody>
            {conducesToDisplay.length > 0 ? (
              conducesToDisplay.map((shipment) => (
                <ShipmentTableRow 
                  key={`${shipment.conduce_number}-${shipment.bulto_sequence}`} 
                  shipment={shipment}
...
export default BultosVerificadosTable;
