...
import { fetchAppConfig, getMapTileUrl } from '@/services/configService';
...
export const MapaChoferEntregas: React.FC<MapaChoferEntregasProps> = ({ 
  ...
  const [savedStopIndex, setSavedStopIndex] = useState<number>(() => {
  ...
  const [isNavigating, setIsNavigating] = useState(() => {
    ...
    try {
      hasValidRoute = savedRoute ? JSON.parse(savedRoute).length > 0 : false;
    } catch {
      hasValidRoute = false;
    }
    if (!hasValidRoute) {
  ...
  const lastRenderedMarkersKeyRef = useRef<string>('');
  const lastRouteCoordsKeyRef = useRef<string>('');
  ...
  // Auto-generar la ruta optimizada automáticamente si hay clientes con GPS y aún no hay ruta creada
  const autoGeneratedRouteRef = useRef(false);
  useEffect(() => {
    if (!autoGeneratedRouteRef.current && clientsWithCoordinates.length > 0 && optimizedRoute.length === 0) {
      autoGeneratedRouteRef.current = true;
      const startLat = clientsWithCoordinates[0].lat;
      const startLon = clientsWithCoordinates[0].lon;
      runRouteOptimization(startLat, startLon, true);
    }
  }, [clientsWithCoordinates.length, optimizedRoute.length]);
  ...
  const handleStartRoute = async (navApp: 'app' | 'google' | 'waze' = 'app') => {
    ...
    try {
      ...
      localStorage.setItem('active_route_started', 'true');
  ...
  const handleResumeRoute = () => {
    ...
    const saved = localStorage.getItem('nav_current_stop');
    if (saved) {
      setSavedStopIndex(parseInt(saved, 10));
    }
    setIsNavigating(true);
  };
  ...
  const handleClearRoute = () => {
    ...
    toast({
      ...
      description: "Se ha eliminado la ruta en memoria para crear una nueva.",
    });
  };
  ...
  useEffect(() => {
    ...
    const getTileUrl = () => getMapTileUrl(isDark());

    // Capa de calles (CartoDB o OpenStreetMap)
  ...
  // 2. Renderizar y actualizar marcadores en el mapa Leaflet de forma dinámica y sin parpadeo
  useEffect(() => {
        ...
        const newMarkers = clientsWithCoords.map(client => {
          ...
          const customIcon = L.divIcon({
            ...
            html: `<div style="
              ...
              justify-content: center;
            ">
              <!-- Aura estática elegante detrás de la casita (sin parpadeo) -->
              <div style="
          ...
          const tooltipContent = `
            <div style="font-family: sans-serif; font-size: 11px; padding: 2px; line-height: 1.4; pointer-events: none;">
    ...
    if (routeViewMode === 'optimized' && userLocation) {
      // 1. Marcador de ubicación del usuario (solo actualizar posición sin destruir)
      if (!userMarkerRef.current) {
        ...
        userMarkerRef.current = L.marker([userLocation.lat, userLocation.lon], { icon: userIcon })
          .addTo(map)
          .bindTooltip("Mi Ubicación Actual", { direction: 'top', offset: [0, -12], interactive: false });
      } else {
        userMarkerRef.current.setLatLng([userLocation.lat, userLocation.lon]);
      }

      // 2. Línea de ruta real usando OSRM API (con cache para evitar parpadeos al re-renderizar)
      const points = [
        {lat: userLocation.lat, lon: userLocation.lon},
        ...optimizedRoute.map(c => ({lat: c.lat, lon: c.lon}))
      ];
      const coordsString = points.map(p => `${p.lat.toFixed(5)},${p.lon.toFixed(5)}`).join(';');

      if (lastRouteCoordsKeyRef.current !== coordsString) {
        lastRouteCoordsKeyRef.current = coordsString;
        let isSubscribed = true;

        const fetchRealRoute = async () => {
          try {
            const osrmPoints = points.map(p => `${p.lon},${p.lat}`).join(';');
  ...
  return (
    <>
                    ...
                    >
                      ...
                      {projectedClients.map((client) => {
                        ...
                        return (
                          ...
                          >
                            {/* Aura estática royal-yellow (sin parpadeo) */}
                            <circle 
                              cx={client.x} 
                              cy={client.y - 13} 
                              r="13" 
                              className="fill-[#f5a623]/25 group-hover:fill-[#f5a623]/45 transition-colors"
                            />
                            {/* Pin de Mapa en SVG */}
                            <g 
                    ...
                    {optimizedRoute.length > 0 && (
                      <div className="p-2 border-b border-border/40">
                        {isActiveRouteSuspended ? (
                          <Button 
                            onClick={handleResumeRoute} 
                            className="w-full bg-royal-blue hover:bg-royal-blue/90 text-white font-bold h-10 shadow"
                            size="sm"
                          >
                            <Navigation className="mr-2 h-4 w-4 fill-current" /> Continuar Navegación (Parada {savedStopIndex + 1})
                          </Button>
                        ) : (
                          <Button 
      ...
      {isNavigating && optimizedRoute.length > 0 && createPortal(
        <DeliveryNavigationMode
          ...
          onClose={(finished?: boolean) => {
            ...
            localStorage.removeItem('is_navigating');
            const currentStop = localStorage.getItem('nav_current_stop');
            if (currentStop) {
              setSavedStopIndex(parseInt(currentStop, 10));
            }
